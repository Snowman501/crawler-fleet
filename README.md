...
`crawler.py` is the original local-business prototype; its old scores and outreach drafts are not validated. `fleet.py` does not depend on Ollama. It records the final URL, timestamp, page title, description, and six narrowly scoped checks. A request failure produces no speculative report.
